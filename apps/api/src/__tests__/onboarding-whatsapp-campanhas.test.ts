...
describe('onboarding-whatsapp — flavor campanhas', () => {
  ...
  it('closeLeadOnConnected manda a mensagem de Campanhas pela instância certa quando lead.source="campanhas"', async () => {
    leads.set('5511900001111', { phone: '5511900001111', stage: 'code_sent', numberId: 'meu_numero', name: null, pushName: 'Ana', source: 'campanhas' });
    await closeLeadOnConnected('meu_numero');
    expect(leads.get('5511900001111').stage).toBe('completed');
    expect(lastSendArgs()[0]).toBe('inst_campanhas');
    expect(lastSendArgs()[2]).toMatch(/campanha nova/);
  });

  it('closeLeadOnConnected mantém a mensagem genérica quando lead.source="oficial"', async () => {
