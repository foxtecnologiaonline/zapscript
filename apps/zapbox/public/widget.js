/**
 * ZapBox — script embutível, plug-and-play.
 *
...
(function () {
  ...
  if (!PUBLIC_KEY) {
    console.error('[ZapBox] data-key ausente no <script>. Widget não iniciado.');
    return;
  }
  ...
  var VISITOR_KEY = 'zb_visitor_' + PUBLIC_KEY;
  var CONTACT_KEY = 'zb_contact_' + PUBLIC_KEY;
  ...
  async function api(path, opts) {
    ...
    if (!res.ok) throw new Error('zapbox_api_error_' + res.status);
    return res.json();
  }
  ...
  style.textContent = [
    ':host { all: initial; }',
    '.zb-bubble { position: fixed; bottom: 20px; ' + POSITION + ': 20px; width: 60px; height: 60px;',
    ...
    '.zb-bubble:hover { transform: scale(1.06); }',
    '.zb-bubble svg { width: 28px; height: 28px; fill: #fff; }',
    '.zb-panel { position: fixed; bottom: 92px; ' + POSITION + ': 20px; width: 340px; max-width: calc(100vw - 40px);',
    ...
    '.zb-panel.open { display: flex; }',
    '.zb-header { background: ' + COLOR + '; color: #fff; padding: 14px 16px; font-size: 15px; font-weight: 600; }',
    '.zb-body { flex: 1; overflow-y: auto; padding: 12px; background: #f5f6f8; }',
    '.zb-form { padding: 16px; display: flex; flex-direction: column; gap: 10px; }',
    '.zb-form input { padding: 10px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; }',
    '.zb-form button, .zb-send { background: ' + COLOR + '; color: #fff; border: none; border-radius: 8px;',
    '  padding: 10px 14px; font-size: 14px; font-weight: 600; cursor: pointer; }',
    '.zb-msg { max-width: 80%; padding: 8px 12px; border-radius: 10px; margin-bottom: 8px; font-size: 13px; line-height: 1.4; word-wrap: break-word; }',
    '.zb-msg.out { background: ' + COLOR + '; color: #fff; margin-left: auto; border-bottom-right-radius: 2px; }',
    '.zb-msg.in { background: #fff; color: #222; border: 1px solid #e5e5e5; margin-right: auto; border-bottom-left-radius: 2px; }',
    '.zb-inputRow { display: flex; gap: 8px; padding: 10px; border-top: 1px solid #eee; }',
    '.zb-inputRow input { flex: 1; padding: 10px 12px; border: 1px solid #ddd; border-radius: 20px; font-size: 14px; }',
    '.zb-inputRow button { width: 40px; height: 40px; border-radius: 50%; flex-shrink: 0; }',
  ].join('\n');
  root.appendChild(style);

  var bubble = document.createElement('div');
  bubble.className = 'zb-bubble';
  ...
  panel.innerHTML =
    '<div class="zb-header">Fale conosco</div>' +
    '<div class="zb-body" id="zb-body"></div>' +
    '<div class="zb-inputRow" id="zb-inputRow" style="display:none">' +
    '  <input id="zb-input" type="text" placeholder="Digite sua mensagem..." />' +
    '  <button class="zb-send" id="zb-send">➤</button>' +
    '</div>';
  root.appendChild(panel);

  var bodyEl = panel.querySelector('#zb-body');
  var inputRow = panel.querySelector('#zb-inputRow');
  var open = false;
  ...
  function renderForm() {
    ...
    form.className = 'zb-form';
    form.innerHTML =
      '<input id="zb-name" type="text" placeholder="Seu nome" />' +
      '<input id="zb-phone" type="tel" placeholder="Seu WhatsApp (com DDD)" />' +
      '<button id="zb-start">Iniciar conversa</button>';
    bodyEl.appendChild(form);

    form.querySelector('#zb-start').addEventListener('click', async function () {
      var name = form.querySelector('#zb-name').value.trim();
      var phone = form.querySelector('#zb-phone').value.trim();
  ...
  async function startConversation(name, phone) {
    try {
      ...
      bodyEl.innerHTML = '<div class="zb-msg in">Não foi possível iniciar a conversa. Tente novamente em instantes.</div>';
    }
  }
  ...
  async function send() {
    var input = panel.querySelector('#zb-input');
  ...
  panel.querySelector('#zb-send').addEventListener('click', send);
  panel.querySelector('#zb-input').addEventListener('keydown', function (e) {
    if (e.key === 'Enter') send();
  });
