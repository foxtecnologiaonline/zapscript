...
export function DemoTranscribe() {
  ...
  function reset() {
    setFile(null); setResult(null); setError(null); setEmail(''); setIsExample(false); setMagicSent(false);
    if (inputRef.current) inputRef.current.value = '';
  }

  // Fast-path: magic link direto. Usuário já provou valor — 1 clique, sem senha.
  async function handleMagicRegister() {
    ...
    setError(null);
    try {
      await api.post('/auth/magic-register', { email: email.trim().toLowerCase() });
      setMagicSent(true);
    } catch (e: any) {
        ...
        <div style={{ animation: 'fadeInUp .4s ease both' }}>
          ...
          <div className="rounded-2xl p-4 mb-3" style={{ background: 'rgb(var(--color-surface))', border: '1px solid rgba(16,185,129,.2)' }}>
            ...
            <p className="text-xs font-light mb-3" style={{ color: 'rgb(var(--color-text-secondary))' }}>
              Conecte seu número e todo áudio vira texto e resumo sozinho. Comece grátis — sem cartão.
            </p>
            ...
            {magicSent ? (
              ...
              <div className="space-y-3">
                <div className="text-center py-3 px-4 rounded-xl"
                  ...
                  <p className="text-[12px] mt-1 leading-relaxed" style={{ color: 'rgb(var(--color-text-secondary))' }}>
                    Enviamos um link mágico para <strong style={{ color: 'rgb(var(--color-primary))' }}>{email.trim().toLowerCase()}</strong>.
                    Clique nele e acesse seu painel — sem senha, sem burocracia.
                  </p>
                </div>
                <Link href={signupHref} className="w-full py-2.5 rounded-2xl text-sm font-semibold flex items-center justify-center gap-2 transition-all"
                  style={{ border: '1.5px solid rgb(var(--color-border))', color: 'rgb(var(--color-text-secondary))', background: 'transparent' }}>
                  Abrir página de cadastro completo
                </Link>
              </div>
            ) : (
              /* ── Opções de cadastro ── */
              <div className="space-y-2">
                ...
                <button type="button" onClick={handleMagicRegister} disabled={magicSending}
                  ...
                  style={{ opacity: magicSending ? 0.7 : 1 }}>
                  {magicSending ? 'Enviando link…' : '⚡ Criar conta em 1 clique'}
                </button>
