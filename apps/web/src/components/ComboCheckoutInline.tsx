...
/* ─────────────────────────────────────────────────────────
   ComboCheckoutInline — Checkout Transparente Asaas p/ o Combo
   Mirror leve de ModuleCheckoutInline.tsx (que é por módulo avulso).
   Métodos: PIX · Crédito. Chama POST /billing/combo/subscribe.
   ───────────────────────────────────────────────────────── */

interface ComboModule {
  key:          string;
  name:         string;
  priceMonthly: number;
}
...
/* ── Formatação (mesmas regras do ModuleCheckoutInline) ── */
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ background: 'rgba(var(--color-primary)/.06)', border: '1px solid rgba(var(--color-primary)/.2)', borderRadius: 10, padding: '12px 14px', fontSize: 12, color: 'rgb(var(--color-text-secondary))' }}>
        ...
        O Combo é <strong>ativado automaticamente</strong> após a confirmação.
      </div>
...
function PixGenerateArea({ loading, error, onGenerate, priceLabel }: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ background: 'rgba(var(--color-primary)/.06)', border: '1px solid rgba(var(--color-primary)/.2)', borderRadius: 10, padding: '12px 14px', fontSize: 12, color: 'rgb(var(--color-text-secondary))' }}>
        Gere um QR code PIX para contratar o Combo agora. A cobrança é proporcional ao período restante do seu ciclo atual.
      </div>
      {error && (
        <div style={{ background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#f87171' }}>
          {error}
...
/* ═══════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════ */
export default function ComboCheckoutInline({ corePlanLabel, modules, discountPct, rawTotal, newTotal, onSuccess, onCancel }: Props) {
  ...
  const includedLabel = [corePlanLabel, ...modules.map(m => m.name)].join(' + ');
  const priceLabel     = formatBrl(newTotal);
  ...
  async function handleCardSubmit(card: {
    ...
    try {
      ...
      const res = await api.post<any>('/billing/combo/subscribe', {
  ...
  async function handlePixSubmit() {
    setLoading(true); setError('');
    try {
      const res = await api.post<any>('/billing/combo/subscribe', { paymentMethod: 'pix' });
      if (res.status === 'active') {
        setSuccessMsg(res.message || '');
        setStep(3);
        setTimeout(() => onSuccess(), 1500);
    ...
    return (
      <div style={{ padding: '32px 24px', textAlign: 'center', background: 'rgba(var(--color-primary)/.05)', borderRadius: 14, border: '1px solid rgba(var(--color-primary)/.2)' }}>
        ...
        <div style={{ fontSize: 13, color: 'rgb(var(--color-text-secondary))' }}>
          {successMsg || <><strong>{includedLabel}</strong> estão ativos. Redirecionando...</>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Resumo do combo */}
      <div style={{ background: 'rgb(var(--color-surface))', border: '1px solid rgba(var(--color-primary)/.15)', borderRadius: '12px 12px 0 0', padding: '12px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: 'none', gap: 12 }}>
        <div>
          <div style={{ fontSize: 10, color: 'rgb(var(--color-text-muted))' }}>Contratando o Combo</div>
          <div style={{ fontSize: 15, fontWeight: 800, color: 'rgb(var(--color-text))' }}>{includedLabel}</div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, justifyContent: 'flex-end' }}>
            <span style={{ fontSize: 11, color: 'rgb(var(--color-text-muted))', textDecoration: 'line-through' }}>{formatBrl(rawTotal)}</span>
            <span style={{ fontSize: 15, fontWeight: 600, color: 'rgb(var(--color-text-secondary))' }}>{formatBrl(newTotal)}</span>
            <span style={{ fontSize: 11, color: 'rgb(var(--color-text-muted))' }}>/mês</span>
          </div>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#10b981' }}>
            -{Math.round(discountPct * 100)}% no combo
          </div>
        </div>
      </div>
        ...
        <div style={{ padding: '20px 18px', minHeight: 260, display: 'flex', flexDirection: 'column' }}>
          ...
          {method === 'credit_card' && (
            <CardForm
              ...
              loading={loading}
              error={error}
              submitLabel={`Contratar Combo — ${priceLabel}/mês`}
            />
          )}
