...
export default function SmartFAQAssistant({ onSuggestionsReceived }: SmartFAQAssistantProps) {
  ...
  return (
    <div className="rounded-xl border border-blue-800/60 bg-blue-950/20 p-6">
      ...
      {/* Pergunta */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-brand-text-secondary mb-2">Pergunta</label>
        <input
          type="text"
          value={question}
          onChange={(e) => {
          ...
          maxLength={150}
          className="w-full rounded-lg border border-brand-border bg-brand-elevated px-3 py-2 text-sm text-brand-text placeholder-neutral-600 focus:outline-none focus:border-blue-500"
        />
      </div>

      {/* Resposta */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-brand-text-secondary mb-2">Resposta</label>
        <textarea
          value={answer}
          onChange={(e) => {
          ...
          maxLength={500}
          rows={3}
          className="w-full rounded-lg border border-brand-border bg-brand-elevated px-3 py-2 text-sm text-brand-text placeholder-neutral-600 focus:outline-none focus:border-blue-500 resize-y"
        />
      </div>
      ...
      {suggestions.length > 0 && (
        <div className="mt-4 space-y-2">
          ...
          {suggestions.map((s, i) => (
            <div
              key={i}
              className="rounded-lg border border-blue-800/40 bg-blue-950/30 p-3 text-xs text-brand-text-secondary cursor-pointer hover:bg-blue-950/50 transition-colors"
