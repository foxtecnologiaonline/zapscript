  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl">
      <div className="flex items-start justify-between mb-6 gap-3 flex-wrap">
        ...
        <button onClick={() => setShowUpload(true)} className="btn-primary text-sm px-4 py-2.5 flex items-center gap-2 flex-shrink-0">
          <span>🎙️</span><span className="hidden xs:inline sm:inline">Enviar áudio</span><span className="xs:hidden sm:hidden">Enviar</span>
        </button>
      </div>
          ...
          items.map(t => (
            <div key={t.id}
              ...
              <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setSelected(t)}>
                {/* Linha 1: nome + data (date à direita no mobile, badge adicional no desktop) */}
                ...
                {/* Linha 2: badge do número (sempre visível) */}
          ...
          <div className="modal-panel w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl max-h-[88vh] overflow-y-auto"
            ...
            <div className="flex items-start justify-between mb-4 p-5 pb-0">
              ...
              <button onClick={() => setSelected(null)}
                className="text-brand-muted hover:text-brand-text text-xl leading-none transition-colors p-1">✕</button>
            </div>
