...
// ─────────────────────────────────────────────────────────────────
//  PIPELINE VN — Notas de Voz gravadas no browser
//  source='voice-note', audioBase64 presente (upload via dashboard)
...
async function processVoiceNoteUploadJob(job: Job) {
  ...
  try {
    ...
    // PASSO 8: Enviar transcrição de volta ao próprio WhatsApp (best-effort)
    const instName = instanceName ?? whatsappNumber?.zapiInstanceId;
    const phone    = selfPhone    ?? whatsappNumber?.phoneNumber;

    if (instName && phone && phone !== 'pending') {
      try {
        log(job, `📤 Enviando nota ao próprio número (${phone})...`);
        const message = buildVoiceNoteMessage(bullets, originalText, durationSec);
        await sendMessageViaEvolution(instName, phone, message);
        log(job, '✅ Nota enviada ao WhatsApp');
      } catch (sendErr: any) {
        // Não falha o job se o envio WhatsApp der erro — a transcrição já foi salva
        log(job, `⚠️  Falha ao enviar WhatsApp (non-fatal): ${sendErr.message}`);
      }
    } else {
      log(job, 'ℹ️  Sem número conectado — transcrição salva sem envio WhatsApp');
    }

    // PASSO 9: Notificar dashboard via Socket.IO (fire-and-forget)
    ...
    if (apiUrl && intToken) {
      fetch(`${apiUrl}/internal/emit`, {
        ...
        body: JSON.stringify({
          ...
          data:  { transcriptionId: transcription.id, numberId: whatsappNumber?.id, durationSec, senderName: 'Nota de Voz' },
        }),
        signal: AbortSignal.timeout(5_000),
      }).catch(() => null);
    }

    log(job, `✅ Nota de voz concluída`);
    return { transcriptionId: transcription.id };

  } catch (err) {
    log(job, `❌ Erro na nota de voz: ${(err as Error).message}`);
    throw err;
  } finally {
    mp3Buffer?.fill(0);
    // Limpar arquivo temporário do Supabase Storage (sempre, mesmo em erro)
...
/**
 * Formata mensagem de nota de voz pessoal enviada ao próprio número.
 * Tom mais íntimo — sem "Áudio de X", direto ao ponto.
 */
function buildVoiceNoteMessage(bullets: string[], originalText: string, durationSec: number): string {
  const mode   = audioMode(originalText);
  const durStr = durationSec > 0
    ? (durationSec >= 60
      ? `${Math.floor(durationSec / 60)}m${durationSec % 60 > 0 ? ` ${durationSec % 60}s` : ''}`
      : `${durationSec}s`)
    : '';
  ...
  // Cabeçalho
  const header = tldr
    ? `🎤 *Nota de Voz*${durStr ? ` • ${durStr}` : ''}\n→ ${tldr}`
    : `🎤 *Nota de Voz*${durStr ? ` • ${durStr}` : ''}`;

  // Resumo (só no modo bullets)
  const bulletSection = hasRealBullets && mode === 'bullets'
    ? `\n\n📋 *Resumo*\n${bullets.map(b => `• ${b}`).join('\n')}`
    : '';

  // Transcrição completa (truncada se longa)
  const MAX_CHARS  = 800;
  const truncated  = originalText.length > MAX_CHARS
    ? originalText.slice(0, MAX_CHARS).trimEnd() + '…'
    : originalText;
  ...
  return header + bulletSection + transcSection + '\n\n_ZapScript.me_ ⚡';
}
...
async function routeJob(job: Job) {
  // Nota de voz gravada no browser — tem audioBase64, não tem messageData
  if (job.name === 'transcribe-voice-note') return processVoiceNoteUploadJob(job);
  ...
  // voice-note do WhatsApp (fromMe=true) e whatsapp-evolution → Evolution pipeline
