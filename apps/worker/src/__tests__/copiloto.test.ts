...
import {
  isQuietNow, renderBriefingMessage, renderReplyFooter, readMinConfidence, assignPendingLabel,
  enqueueCopilotoIngest, hasCopiloto,
} from '../copiloto';
 ...
 * Whisper, então não dá pra passar pela rota normal do webhook. jobId/bucket
 * têm que ser IDÊNTICOS aos da versão da API pro dedup/debounce funcionarem
 * igual não importa quem enfileirou.
 */
describe('enqueueCopilotoIngest (v2.1 — áudio transcrito → Copiloto)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockQueueAdd.mockResolvedValue(undefined);
  });

  it('enfileira ingest + brief quando direction é "in"', async () => {
    ...
    expect(mockQueueAdd).toHaveBeenCalledWith(
      'brief',
      { userId: 'user-1', numberId: 'number-1', contactPhone: '5511999999999' },
      expect.objectContaining({ jobId: expect.stringMatching(/^copiloto-brief-number-1-5511999999999-\d+$/) }),
    );
    expect(mockQueueAdd).toHaveBeenCalledTimes(2);
  });

  it('não enfileira brief quando direction é "out" (dono respondendo, não abre janela)', async () => {
    await enqueueCopilotoIngest({
      userId: 'user-1', numberId: 'number-1', contactPhone: '5511999999999',
